...
"시스템 프로그래밍 프로젝트를 진행하며 소스 코드를 5개의 모듈로 분리하다 보니, 가장 까다로웠던 것이 '빌드 의존성 관리'였습니다. 이를 해결하기 위해 Makefile을 직접 작성하여 컴파일 단계와 링킹 단계를 완벽하게 분리했습니다. 특히 단순한 라이브러리 링킹을 넘어, CFLAGS에 -D_XOPEN_SOURCE_EXTENDED=1 같은 전처리기 매크로를 주입하여 컴파일러에게 POSIX 유니코드 확장 표준을 사용하도록 직접 지시했습니다. 결과적으로 make 명령어 하나만으로 복잡한 멀티스레드와 터미널 UI 라이브러리가 엮인 대규모 C 프로젝트를 단 1초 만에 최적화(O2) 빌드해 내는 안정적인 파이프라인을 구축했습니다."