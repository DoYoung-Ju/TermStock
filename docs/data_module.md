...
* **역할:** 사용자가 현재 보고 있는 캔들 차트의 분봉/일봉 기준(Timeframe)을 저장하는 전역 상태 변수입니다.
* **특징:** 기본값은 `"1h"`(1시간봉)로 설정되어 있으며, 메인 UI 스레드에서 사용자의 입력(`1`=1m, `2`=15m, `3`=1h, `4`=1d)에 따라 실시간으로 값이 변경됩니다. 네트워크 스레드는 이 전역 변수를 읽어 API 호출 URL을 동적으로 조립하게 됩니다.
