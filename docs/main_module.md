...
3. **Cleanup (메모리 해제 및 종료):**
   ...
   * `endwin()`을 호출하여 터미널을 원래의 쉘(Shell) 상태로 깔끔하게 복구합니다.
   ...
   ### 🚀 Advanced Features: Responsive UI & Timeframe Control (추가 기능)

* **Dynamic Responsive Scaling (반응형 렌더링):**
  고정된 차트 크기에서 벗어나, `getmaxyx()`로 터미널 창의 실시간 해상도를 감지합니다. 창 크기를 늘리거나 줄일 때마다 전체 높이와 너비를 계산하여 차트가 빈 공간을 100% 활용하도록 수학적 비율을 동적 할당(Dynamic Scaling)합니다. 터미널 창을 조절해도 캔들 차트가 깨지지 않고 HTS처럼 자연스럽게 확장/축소됩니다.
* **Real-time Timeframe Switch (타임프레임 동적 전환):**
  단순한 시세 뷰어를 넘어, 유저가 키보드 숫자키(`1`, `2`, `3`, `4`)를 누르는 즉시 상태 변수(`current_interval`)를 변경하고 차트를 초기화합니다. 1분봉(단기 변동성)부터 1일봉(장기 추세)까지 하나의 터미널 창에서 즉각적으로 넘나드는 트레이딩 분석 툴의 역할을 수행합니다.
